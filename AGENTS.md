...
Beyond editable content, the site takes orders: a customer fills a basket and submits it, and the bakery receives the order as an email. **No order is persisted to the database — the "order" is an email**, and there is no automatic reply to the customer (the confirmation modal says so).

- **Cart state** lives entirely client-side in one React context, `src/components/cart/cart-context.tsx`, persisted to `localStorage` (key `bakkerij-de-tureluur-cart`; carts older than 7 days are discarded on load; per-line quantity capped at 99). Hydration is deliberate — the cart stays empty until a post-paint effect reads storage, so SSR and first client render match. The `bestellen` global's `ordersEnabled` flag is threaded in from `layout.tsx` (`CartProvider isOrderingEnabled=…`) and gates the whole feature; the `CartOverlay` (drawer + confirmation modal) is only mounted when it's on.
- **The client cluster** (`src/components/cart/`, all `'use client'`): `cart-button.tsx` (header basket + badge), `add-to-cart-button.tsx` (on each `product-card.tsx`), `cart-drawer.tsx` (the basket panel, built on the shared `Dialog`), `checkout-form.tsx` (the order form — real `<input>`s + a honeypot + `TurnstileWidget`; it sends only `{ productId, quantity }` to the server, never product names), `order-confirmation-modal.tsx`, `cart-overlay.tsx` (owns the drawer↔modal handoff), `turnstile-widget.tsx`.
- **The server action** `src/backend/actions/submit-order.ts` is the source of truth and is hardened deliberately (this is a public endpoint, so its TypeScript argument type is only a hint): it re-validates the payload shape at runtime, checks the honeypot, re-verifies Turnstile server-side, strips CR/LF from single-line fields (email-header-injection defence), bounds every field/quantity, validates `pickupDay` against the `bestellen` global's allow-list, **re-derives product names/weights from Payload by id** (the email is built from DB data, never client input), and only ever sends to the fixed `ORDER_TO_EMAIL` (so the form can't be used as an open relay). It returns a plain `{ ok }` result instead of throwing. In production with no SMTP configured it fails loudly rather than reporting a success Payload would only console-log — losing the order silently.
- `ordersEnabled` is enforced in three places: the UI (buttons hidden / `CartOverlay` not mounted) and again inside `submitOrder`, so flipping the CMS flag fully closes the feature.
